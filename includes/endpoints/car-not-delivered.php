<?php
/**
 * Car Not Delivered Endpoint
 * 
 * Displays all cars that have not been delivered for the current user
 * 
...
/**
 * Car Not Delivered endpoint class
 */
class Carspace_Endpoint_Car_Not_Delivered extends Carspace_Endpoint {
    /**
     * Constructor
     */
    public function __construct() {
        $this->endpoint = 'car-not-delivered';
        $this->title = __('Not Delivered', 'carspace-dashboard');
        $this->icon = 'hourglass';
        $this->position = 30;
    ...
    protected function filter_car($car) {
        ...
        // Exclude if has booking or is loaded (container number present)
        ...
        // Must have neither featured image nor gallery images
        $has_featured = (bool) $product->get_image_id();
        $gallery_ids  = $product->get_gallery_image_ids();
        $has_gallery  = !empty($gallery_ids);

        return (!$has_featured && !$has_gallery);
    }
    ...
    protected function render_empty_state() {
        echo '<p class="alert alert-success">' . esc_html__('All cars have been delivered!', 'carspace-dashboard') . '</p>';
    }
    ...
    public function render_content() {
        echo '<h3>' . esc_html__('Not Delivered Cars', 'carspace-dashboard') . '</h3>';
...
new Carspace_Endpoint_Car_Not_Delivered();